...
def generate_random_5bit_numbers(count: int) -> list:
    return [''.join(random.choices('01', k=5)) for _ in range(count)]

def main():
    # 生成 100 筆 5 位數字
    random_numbers = generate_random_5bit_numbers(100)

    # 編碼並輸出到 output.txt
    with open('input_5.txt', 'w') as output_file:
        for number in random_numbers:
            encoded_number = hamming_encode(number)
            output_file.write(f"{encoded_number}\n")

    # 將原始數據輸出到 input.txt
    with open('output_5.txt', 'w') as input_file:
        for number in random_numbers:
            input_file.write(f"{number}\n")

    print("已將編碼和原始數據分別輸出至 output.txt 和 input.txt")
