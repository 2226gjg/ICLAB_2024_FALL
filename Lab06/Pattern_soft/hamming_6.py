...
def hamming_encode(data: str) -> str:
    ...
    # 檢查位 2 (p2)
    code[1] = str(int(code[2]) ^ int(code[5]) ^ int(code[6])^ int(code[9]))

    # 檢查位 4 (p4)
    code[3] = str(int(code[4]) ^ int(code[5]) ^ int(code[6]))

    # 檢查位 8 (p8)
    code[7] = str(int(code[8])^ int(code[9]))
...
def generate_random_6bit_numbers(count: int) -> list:
    return [''.join(random.choices('01', k=6)) for _ in range(count)]

def main():
    # 生成 100 筆 6 位數字
    random_numbers = generate_random_6bit_numbers(100)

    # 編碼並輸出到 output.txt
    with open('input_6.txt', 'w') as output_file:
        for number in random_numbers:
            encoded_number = hamming_encode(number)
            output_file.write(f"{encoded_number}\n")

    # 將原始數據輸出到 output.txt
    with open('output_6.txt', 'w') as input_file:
        for number in random_numbers:
            input_file.write(f"{number}\n")

    print("已將編碼和原始數據分別輸出至 input_6.txt 和 output_6.txt")
